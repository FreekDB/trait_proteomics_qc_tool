...
_OUT_DIR = ''
_COPY_LOG = ''
_WEB_DIR = os.path.normpath(r'E:\\Web')
_NIST = os.path.normpath(r'C:\\Users\\nbic\\Documents\\NISTMSQCv1_2_0')
_PROGRES_LOG = r'qc_status.log'
...
def monitor_input():
	...
	for f, status in files.iteritems():
		if status == 'new':
			...
			_RAW_FILE = r'{0}\\{1}'.format(_IN_DIR, f)
			_BASE_NAME = os.path.splitext(f)[0]
			_OUT_DIR = r'{0}\\{1}_QC\\'.format(_OUT_DIR, _BASE_NAME)
...
def _manage_paths():
	...
	today = date.today()
	y = os.path.normpath(r'{0}\\{1}'.format(_WEB_ROOT, today.year))
	m = os.path.normpath(r'{0}\\{1}'.format(today.year, today.month))
	d = os.path.normpath(r'{0}\\{1}'.format(today.month, today.day))
	web = os.path.normpath(r'{0}\\{1}'.format(today.day, _BASE_NAME))
	...
	if not os.path.exists(web):
		os.makedirs(web)
	# TODO: else, folder exists, something went wrong?

	global _WEB_DIR
	_WEB_DIR = web
	
	# Create _OUT_DIR storing NIST output
	project = os.path.dirname(_OUT_DIR)
...
def _run_R_script():
	...
	Rcmd = 'Rscript {0} {1} {2} {3}'.format(_R_GRAPHICS, _RAW_FILE, r'{0}\\{1}'.format(_WEB_DIR, _BASE_NAME), 1)
	_run_command(Rcmd)
	
def _create_metrics():
	""" Parses NIST metrics output file extracting relevant metrics subset """
	metrics_file = os.path.normpath(r'{0}\\{1}_report.msqc'.format(_OUT_DIR, _BASE_NAME))
...
def _create_report():
	# Graphics to include
	heatmap = r'{0}\\{1}_heatmap.pdf'.format(_WEB_DIR, _BASE_NAME)
...
def _run_msconvert():
	...
	mzXML_cmd = r'{0} {1} -o {2} --mzXML -e .RAW.mzXML'.format(msconvert, _RAW_FILE, _OUT_DIR)
	MGF_cmd   = r'{0} {1} -o {2} --mgf -e .RAW.MGF'.format(msconvert, _RAW_FILE, _OUT_DIR)
